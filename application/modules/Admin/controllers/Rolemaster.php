...
class Rolemaster extends CI_Controller {
    ...
    public function index() {
        ...
        if(!empty($searchtext))
        {
            ...
            $match = '';
            $where_search='((rm.role_name LIKE "%'.$searchtext.'%" OR rm.status LIKE "%'.$searchtext.'%")AND rm.is_delete = "0")';
...
//pr($data['campaign_info']);exit;
    ...
    public function edit($id) {
        ...
        $table = ROLE_MASTER . ' as rm';
        $match = "rm.role_id = " . $id;
        $fields = array("rm.role_id, rm.role_name, rm.status, rm.parent_role");
    ...
    public function editPermission($id = null) {
        ...
        if($role_id != $id){
            ...
            if ($id != NULL) {
                ...
                $match_data = "role_id = " . $id;
               $data['parent_role_data'] = $this->common_model->get_records($table1, $fields1, '', '', $match_data);
                $data['parednt_data'] = $data['parent_role_data'][0]['parent_role'];
