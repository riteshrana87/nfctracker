...
class Ibp extends CI_Controller {
    ...
    function __construct() {
        ...
        parent::__construct();
        /*if (checkPermission('Ibp', 'view') == false) {
            redirect('/Dashboard');
        }*/
    ...
    public function edit($id) {
      //get pp form
      $match = array('ibp_form_id'=> 1);
      $pp_forms = $this->common_model->get_records(IBP_FORM,'', '', '', $match);
      if(!empty($pp_forms))
      {
                ...
                if(isset($row['name']))
                {
                    ...
                    { 
                      ...
                      if(!empty($postData['hidden_'.$row['name']]))
                      {
                          ...
                          if(!empty($delete_img))
                          {
                              foreach ($delete_img as $img) {
                                ...
                                if (file_exists ($this->config->item ('ibp_img_url') .$postData['yp_id'].'/'.$img)) { 
                                    unlink ($this->config->item ('ibp_img_url') .$postData['yp_id'].'/'.$img);
                                }
                                if (file_exists ($this->config->item ('ibp_img_url_small') .$postData['yp_id'].'/'.$img)) {
                                    unlink ($this->config->item ('ibp_img_url_small') .$postData['yp_id'].'/'.$img);
                                }
                              } 
                          }
                      }
                      ...
                      if(!empty($_FILES[$filename]['name'][0]))                     
                      {
                          ...
                          //create dir and give permission
                        if (!is_dir($this->config->item('ibp_base_url'))) {
                                mkdir($this->config->item('ibp_base_url'), 0777, TRUE);
                        }

                        if (!is_dir($this->config->item('ibp_base_big_url'))) {                                
                            mkdir($this->config->item('ibp_base_big_url'), 0777, TRUE);
                        }
                        
                        
                        if (!is_dir($this->config->item('ibp_base_big_url') . '/' . $postData['yp_id'])) {
                            mkdir($this->config->item('ibp_base_big_url') . '/' . $postData['yp_id'], 0777, TRUE);
                        }
                          ...
                          if(!empty($upload_data))
                          {
                            foreach ($upload_data as $imageFiles) {
                               if (!is_dir($this->config->item('ibp_base_small_url'))) {                                        
                                    mkdir($this->config->item('ibp_base_small_url'), 0777, TRUE);
                                }
                                
                                if (!is_dir($this->config->item('ibp_base_small_url') . '/' . $postData['yp_id'])) {                                        
                                    mkdir($this->config->item('ibp_base_small_url') . '/' . $postData['yp_id'], 0777, TRUE);
                                }
   ...
   public function archive($id)
   {
            ...
            $data['archivedata'] = $this->common_model->get_records(IBP_ARCHIVE.' as pa',$fields, $join_tables, 'left', $match);
